...
function do_func
  set -l _spring_project spring-cloud-gcp-firestore-gs
  set -l _spring_project_path (pwd |awk -F "/$_spring_project" '{print $1}')/$_spring_project

  argparse -n do_func 'h/help' 'p/project=' 'n/name=' 't/tag=' -- $argv
  or return 1

  if set -lq _flag_help
    echo "generate-gradle-props.fish -p/--project <GCP_PROJECT_ID> -n/--name <APP_NAME> -t/--tag <TAG>"
    return
  end
